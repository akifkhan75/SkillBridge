...
import type { Worker, WorkerPage, WorkerActivationStatus } from '../types';
...
export const WorkerNav: React.FC<WorkerNavProps> = ({ worker, ...props }) => {
  ...
  const getStatusBanner = () => {
    if (workerActivationStatus === 'PENDING_REVIEW') {
      return (
        <StatusBanner style={{ backgroundColor: '#fefce8', borderColor: '#facc15' }}>
    ...
    if (workerActivationStatus === 'NEEDS_VERIFICATION') {
        return (
          <StatusBanner style={{ backgroundColor: '#fff7ed', borderColor: '#fb923c' }}>
              ...
              {...(item.isComingSoon && {
                label: () => (
                    <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                        <Text style={{ color: isFocused ? '#1d4ed8' : '#1f2937', fontWeight: '500', opacity: 0.5, flex: 1 }}>{t(item.labelKey)}</Text>
